...
Trayecto guarda todo en dos archivos JSON (por defecto en `~/.trayecto/`). Puedes escribirlos a mano: al leerlos, Trayecto completa lo que falte (ids, valores por defecto) y descarta lo que no entienda.

## `trayecto.json`: lo activo
...
| `proyectos[].previo` | número ≥ 0 | trabajo hecho antes de llevarlo en Trayecto, en unidades de tarea. Es lo que mueve *Ajustar avance* |
...
- Revertir es el camino inverso. Si regresas una tarea de un proyecto archivado, el proyecto vuelve a `trayecto.json`.
...
pendiente = suma de pesos de sus tareas en trayecto.json
