...
Por defecto solo escucha en `127.0.0.1` y rechaza peticiones con otro `Host` u `Origin`. Con `--red` escucha en toda tu red, y los demás dispositivos deben presentar la clave de `<datos>/clave`: en el encabezado `X-Flecha-Clave`, o abriendo una vez `/?clave=...`, que la deja en una cookie. `GET /api/enlace` (solo desde la misma computadora) devuelve ese enlace completo, y `GET /api/emparejar` un código de 6 dígitos de 10 minutos; el teléfono lo canjea con `POST /api/emparejar {"codigo"}` y recibe la clave. Con `--red` el servidor también se anuncia por Bonjour como `_flecha._tcp` para que la app lo encuentre sola.
